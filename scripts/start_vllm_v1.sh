...
# Ensure repo root as working directory so module paths/config.json resolve.
...
if [[ "${USE_VLLM_V1:-0}" == "1" || "${VLLM_USE_V1:-0}" == "1" ]]; then
  KV_JSON='{"kv_connector":"DKVEngineConnectorV1","kv_connector_module_path":"distributed_kv_manager.vllm_adapter.dkv_offloading_connector_v1","kv_role":"kv_both","kv_connector_extra_config":{"config_path":"config_v1.json"}}'
...
if [[ "${USE_VLLM_V1}" == "1" ]]; then
  ...
  exec bash scripts/start_vllm_v0.sh
fi
